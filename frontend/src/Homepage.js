...
export default function Homepage({ user, onLogout }) {
  ...
  const checkManagerStatus = async () => {
    try {
      ...
      const response = await fetch('https://urwall-production-7ba9.up.railway.app/api/auth/check-manager', {
        headers: {
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://urwall-production-7ba9.up.railway.app/api/posts', {
        method: 'POST',
        headers: {
